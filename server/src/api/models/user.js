...
async function create(user) {
  ...
  const data = await sql.query(
    "INSERT INTO tmp_user (name, gender, avatar_index, internal_user_id) VALUES (?, ?, ?, ?);",
    [user.name, user.gender, user.avatarIndex, user.id]
  );
  return data[0];
}
...
async function query(id) {
  const data = await sql.query(
    "SELECT * FROM tmp_user WHERE internal_user_id = ?;",
    [id]
  );
  return data[0];
}
...
async function update(user) {
  ...
  const data = await sql.query(
    "UPDATE tmp_user SET name = ?, gender = ?, avatar_index = ?, birth_date = ? WHERE internal_user_id = ?;",
    [user.name, user.gender, user.avatarIndex, user.dob, user.id]
  );
  return data[0];
}
...
async function createReal(user) {
  ...
  user.gender = user.gender.charAt(0).toUpperCase() + user.gender.slice(1);
  const data = await sql.query(
    "INSERT INTO user (email, password, name, gender, avatar_index, internal_user_id) VALUES (?, ?, ?, ?, ?, ?);",
    [
      user.email,
      user.password,
      user.name,
...
async function queryReal(id) {
  const data = await sql.query(
    "SELECT * FROM user WHERE internal_user_id = ?;",
    [id]
  );
  return data[0];
}
...
async function deleteRealUser(id) {
  const data = await sql.query("DELETE FROM user WHERE internal_user_id = ?;", [
    id,
  ]);
  return data[0];
}
...
async function disableRealUser(id) {
  ...
  let hashEmail = crypto
    .createHmac("md5", process.env["MD5_SECRET_KEY"])
    .update(newEmail)
    .digest("hex");
  ...
  const data = await sql.query(
    "UPDATE user SET internal_user_id = ?, email = ? WHERE id = ?;",
    [hashEmail, newEmail, user.id]
  );
  return data[0];
}
...
async function deleteTmpUser(id) {
  const data = await sql.query(
    "DELETE FROM tmp_user WHERE internal_user_id = ?;",
    [id]
  );
  return data[0];
}
...
async function updateReal(user) {
  ...
  const queryResults = await sql.query(
    "SELECT * FROM user WHERE internal_user_id = ?;",
    [user.id]
  );
  ...
  const avatarIndex =
    user.avatarIndex != undefined ? user.avatarIndex : oldUsers[0].avatar_index;
  const emojiIndex =
    user.emojiIndex != undefined ? user.emojiIndex : oldUsers[0].emoji_index;
  ...
  let is_daily_quest_notification_on =
    oldUsers[0].is_daily_quest_notification_on;
  ...
  const data = await sql.query(
    "UPDATE user SET password = ?, name = ?, gender = ?, avatar_index = ?, emoji_index = ?, status = ?, birth_date = ? , is_notification_on = ?,is_match_card_notification_on = ?,is_daily_quest_notification_on = ?, is_new_blog_notification_on = ? WHERE internal_user_id = ?;",
    [
      password,
      name,
      gender,
