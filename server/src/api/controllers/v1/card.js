...
const zodiacData = JSON.parse(
  fs.readFileSync(path.join(dir, "../../../../static/zodiac_card_data.json"))
);
const animalData = JSON.parse(
  fs.readFileSync(
    path.join(dir, "../../../../static/chinese_zodiac_card_data.json")
  )
);
...
async function getUserCard(req, res) {
          ...
          matchData.map(async (match) => {
            ...
            if (macthUserAvatar.length > 0) {
              return {
                ...
                matchedUserQuadra:
                  data[matchedTarockResult[0].tarock_socionics]
                    .personality_socionic_quadra,
              };
            } else {
              return {
                id: match.id,
  ...
  if (result.length >= 0) {
    if (result[0].type == "Tarock") {
      let zodiac = await userZodiac.getUserZodiac(
      ...
      let animal = await userZodiac.getUserZodiac(
        user.internal_user_id,
        "CHINESE_ZODIAC"
      );
      if (animal.length > 0) {
...
async function getTypeCard(req, res) {
  ...
  let isRated = false;
  if (owner_id) {
    let userRate = await UserRateModel.findByIds(
      owner_id,
...
async function addCard(req, res) {
  ...
  try {
    if (card_type == "ZODIAC") {
      ...
      let small_map = date_map.filter(
        (date) =>
          date.start_date.split("-")[0] == check_month ||
          date.end_date.split("-")[0] == check_month
      );
      ...
      small_map = small_map.filter((date) => {
        if (check_month == 1 && check_date >= 1 && check_date <= 19) {
          ...
          if (date.start_date.split("-")[0] == 12) {
            start_date = new Date(
              zodiac_birth_year - 1 + "-" + date.start_date
            );
          } else {
          ...
          if (
            zodiac_birth_date >= start_date &&
            zodiac_birth_date <= end_date
          ) {
            return true;
          }
          return false;
          ...
          if (check_month == 12 && check_date >= 22 && check_date <= 31) {
            ...
            let start_date = new Date(
              zodiac_birth_year + "-" + date.start_date
            );
            let end_date = new Date(zodiac_birth_year + "-" + date.end_date);
            if (
              zodiac_birth_date >= start_date &&
      ...
      data = await userZodiac.createCard(
        user.internal_user_id,
        card_type,
        gender,
        birth_date,
