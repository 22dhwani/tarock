...
import userPoints from "../../controllers/v1/userPoints.js";
...
export default function (app) {
  ...
  try {
    ...
    router.post(
      "/auth/explore/comments/like-toggle",
      exploreComment.likeToggle
    );
    ...
    //DAILY QUESTIONS
    router.get(
      "/auth/daily-question/get-today-question",
      dailyQuestion.getTodayQuestion
    );
    router.post("/auth/daily-question/submit-answer", dailyQuestion.answer);
    router.get(
      "/auth/daily-question/get-submited-answer",
      dailyQuestion.getAnswer
    );
    router.get(
      "/auth/daily-question/get-question-stats",
      dailyQuestion.getStats
    );
    router.get(
      "/auth/daily-question/get-option-stats",
      dailyQuestion.getOptionStats
    );

    //DAILY QUESTION COMMENTS
    router.get("/auth/daily-question/comments", dailyQuestionComment.index);
    router.post(
      "/auth/daily-question/comments/add",
      dailyQuestionComment.addComment
    );
    router.post(
      "/auth/daily-question/comments/like-toggle",
      dailyQuestionComment.likeToggle
    );
    ...
    //user-points
    router.post("/auth/user-points/add", userPoints.addUserPoint);
    router.get("/auth/user-points/list", userPoints.userPointsList);
    router.get("/auth/user-points/total", userPoints.totalUserPoints);
    router.get("/auth/user-points/daily-check-in", userPoints.dailyLoginPoint);
