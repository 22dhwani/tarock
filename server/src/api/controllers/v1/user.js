...
async function createTempUser(req, res) {
  ...
  try {
    ...
    if (existingUser.length > 0) {
      ...
      await UserAvatarModel.updateAvatar(
        userAvatar[0].id,
        face_index,
        hair_index,
        eyebrow_index,
...
async function createRealUser(req, res) {
  ...
  try {
    ...
    if (req.body.firebase_id) {
      let userFirebase = await UserFirebaseModel.checkExists(
        data[0].internal_user_id,
        req.body.firebase_id
      );
      if (userFirebase.length <= 0) {
        await UserFirebaseModel.addToken(
          data[0].internal_user_id,
          req.body.firebase_id
        );
      }
    ...
    let userAvatar = await UserAvatarModel.getByUserId(
      data[0].internal_user_id
    );
...
async function logout(req, res) {
  ...
  try {
    if (req.body.firebase_id) {
      await UserFirebaseModel.removeToken(
...
async function editUser(req, res) {
  ...
  if (req.body.is_daily_quest_notification_on) {
    user.is_daily_quest_notification_on =
      req.body.is_daily_quest_notification_on;
  } else {
  ...
  user[0].gender =
    user[0].gender.charAt(0).toUpperCase() + user[0].gender.slice(1);
  res.json({
    data: user[0],
...
async function forgotPassword(req, res) {
  ...
  const url =
    ...
    `/password/form?id=${id}&credential=${credential}`;
  const mailOptions = {
    from: sender.email,
    to: email,
  ...
  if (sendToAll) {
    ...
    for await (const user of allUsers) {
      if (user.is_notification_on && user.is_new_blog_notification_on) {
        await Notification.sendToUserId(
          user.internal_user_id,
          null,
          "NEW_BLOG",
          "New blog added, check it out",
          link,
          0,
          blog_id
        );
...
async function socialLogin(req, res) {
  ...
  try {
    ...
    if (provider == "apple") {
      const json = jwt.decode(bearerToken, { complete: true });
      ...
      finalData = await axios.get(
        `https://www.googleapis.com/oauth2/v1/userinfo?id_token=${bearerToken}`,
        {
          headers: {
    ...
    if (emailExistUser.length > 0) {
      let token = await ApiToken.generateToken(
        emailExistUser[0].internal_user_id
      );
