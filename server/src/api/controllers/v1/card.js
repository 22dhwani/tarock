...
async function addCard(req, res) {
    ...
    try {
        if (card_type == "ZODIAC") {
            ...
            let characteristics = user.characteristics
            characteristics[7] = selected_zodiac

            let userHaveCard = await userZodiac.getUserZodiac(user.internal_user_id, card_type)
            console.log(userHaveCard);
            if (userHaveCard.length > 0) {
                //TODO::code to update card
                await User.updateCharacteristics(user.internal_user_id, JSON.stringify(characteristics))
                data = await userZodiac.updateCard(user.internal_user_id, card_type, gender, birth_date, selected_zodiac, null)
                data = await userZodiac.getById(userHaveCard[0].id)
            } else {
                //TODO::code to add card
                await User.updateCharacteristics(user.internal_user_id, JSON.stringify(characteristics))
            ...
            if (userHaveCard.length > 0) {
                //TODO::code to update card
                await User.updateCharacteristics(user.internal_user_id, JSON.stringify(characteristics))
                data = await userZodiac.updateCard(user.internal_user_id, card_type, gender, birth_year, null, selected_animal)
                data = await userZodiac.getById(userHaveCard[0].id)
            } else {
                await User.updateCharacteristics(user.internal_user_id, JSON.stringify(characteristics))
